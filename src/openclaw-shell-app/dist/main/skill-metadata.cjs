...
function parseTopLevelFrontmatter(frontmatter) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (/^[|>][+-]?(?:\d+)?(?:\s+#.*)?$/.test(rawValue)) {
      ...
      result[key] = rawValue.startsWith(">") ? foldYamlBlock(normalized) : normalized.join("\n").trim();
      continue;
    }
...
function parseSkillMeta(skillFilePath) {
  try {
    ...
    if (!emoji) emoji = frontmatter.match(/(?:^|\n)\s*emoji:\s*["']?([^"'\r\n#]+)["']?/i)?.[1]?.trim() || null;
...
function discoverSkillPackages(rootDir, maxDepth = 6) {
  ...
  const topLevelDirectories = rootEntries.filter((entry) => entry.isDirectory() && !entry.name.startsWith("."));
  const packages = [];
  ...
  while (queue.length) {
    ...
    for (const entry of entries) {
      if (!entry.isDirectory() || entry.name.startsWith(".") || EXCLUDED_DIRECTORIES.has(entry.name)) continue;
      queue.push({ directory: path.join(current.directory, entry.name), depth: current.depth + 1, topLevel: current.topLevel });
    }
  }
  ...
  const invalidDirectories = topLevelDirectories
    ...
    .map((entry) => ({ name: entry.name, path: path.join(rootDir, entry.name), reason: "missing-skill-md" }));
...
if (require.main === module) {
  ...
  process.stdout.write(`${JSON.stringify({ ok: true, packages, invalidDirectories: discovery.invalidDirectories })}\n`);
}
