...
function patchHermesRuntimeEnv(filePath) {
  ...
  const chatReplacement = [
    ...
    "        resolve(payload);",
    "      }",
    "      function killForOutput(reason) {",
    "        try { child.kill(\"SIGKILL\"); } catch {}",
    "        const message2 = reason + \"。Hermes 本次任务输出过大，已自动终止以保护桌面程序不被卡死。请把任务拆小，或在 Hermes Dashboard/终端中执行长任务。\";",
    "        safeSend(\"hermes-log\", { type: \"stderr\", msg: \"[chat-output-limit] \" + message2 });",
    "        finish({ ok: false, error: message2 });",
    "      }",
    "      const timer = setTimeout(() => {",
    "        try {",
    "          child.kill(\"SIGKILL\");",
    ...
    "        if (stdoutBytes > maxStdoutBytes * 2) killForOutput(\"stdout 超过 \" + Math.round(stdoutBytes / 1024) + \"KB\");",
    "      });",
    ...
    "        }",
    "        if (stderrBytes > maxStderrBytes * 2) killForOutput(\"stderr 超过 \" + Math.round(stderrBytes / 1024) + \"KB\");",
    "      });",
    "      child.on(\"error\", (err) => {",
    ...
    "          return;",
    "        }",
    "        finish({ ok: true, reply, raw: stdout });",
    "      });",
    "    });",
    "  }"
  ].join("\n");
